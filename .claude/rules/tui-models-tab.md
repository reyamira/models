...
2. **Capabilities** — 2-column `two_pair_line` layout: Reasoning/Tools, Source/Files, Temp/**Structured**. `Structured` renders from `Model.structured_output` (`Option<bool>`, ~49% coverage) via a three-state `cap_val_opt` — Yes (Cyan) / No (DarkGray) / `—` (DarkGray, unknown-when-absent — this is why it lives here and **not** in the compact RTFO row, which is binary-only and stays 4-char `RTFO`). When the model carries `reasoning_options`, a blank line + a **`Reasoning controls:`** label follows, then one indented line per control from `Model::reasoning_controls()` — the API knobs for *controlling* reasoning (distinct from the Reasoning capability flag and the Thinking price). Controls are **split** (one per line, not comma-crammed) because ~474 models expose 2–3 of them. Formatting: `budget {min}–{max}` (`budget_tokens` humanized to `budget`, ranges rounded via `format_tokens`), `effort {level, level, …}` (from the option's `values[]`, `null` "off" entries dropped), `toggle` (bare). Control names are left-padded so details align. An unknown future `type` shows its raw name (permissive). Omitted entirely when the model has no `reasoning_options`
