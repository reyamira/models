...
### Tabs
- **Models Tab** (`src/tui/models/`) — browse models from models.dev API. **Grouped default view**: one row per models.dev canonical model, plus one row per explicitly unlinked offering, with Model | Lab | Providers | price/context ranges; Enter drills into a canonical model's per-provider offerings (breadcrumb + Esc back), `V` toggles the flat per-offering list (persisted), `p` opens a search-first provider modal (the provider sidebar is retired; layout is 60% list | 40% detail). Canonical identity mirrors models.dev's repository resolver exactly: generated provider-TOML `base_model` refs → direct `model_id` → `provider_id/model_id` → unlinked. RTFO capability indicators (majority + dim-when-mixed on grouped rows); duplicate-row dimming in flat views is paused (grouping communicates sameness structurally)
...
### Data Flow
- Model data: the TUI fetches `catalog.json` at startup and on `r` refresh (`src/api.rs`) so provider offerings and the canonical model registry arrive as one coherent snapshot; provider-only CLI commands use the smaller `api.json`. Because every served JSON strips provider TOML `base_model`, `data/models-dev-base-model-refs.json` supplies that edge from a pinned upstream commit. `mise run refresh-model-refs` clones models.dev `dev` and regenerates the artifact through `transform models-dev-refs`; the app accepts only refs whose targets exist in the live catalog, so new/stale entries fail closed rather than falling back to name/slug guesses
...
2. `mise run refresh-model-refs` so the embedded models.dev identity edge is pinned to the latest upstream `dev`
3. `mise run fmt && mise run clippy && mise run test`
4. Commit `Cargo.toml`, `Cargo.lock`, and any refreshed model-reference artifact together
5. `git tag v<version> && git push && git push origin v<version>` — push the release tag **explicitly**; bare `git push --tags` errors benignly here because an old local tag diverges from the remote
6. Release workflow runs automatically: builds binaries, packages .deb/.rpm, publishes to crates.io, and updates AUR package. Homebrew Core bumps happen separately in `Homebrew/homebrew-core`.
7. The GitHub flake is available from the pushed tag automatically (for example, `nix run github:reyamira/models/v<version>`). FlakeHub publishing is manual-only and should only be dispatched after account/org setup is intentionally enabled.
8. Visuals pass: regenerate screenshots/GIFs against the release binary via the VHS tapes in `public/tapes/` with `PATH=$PWD/target/release:$PATH` (screenshots are committed in `public/assets/` and hot-linked by README + wiki; wiki GIFs republish via `vhs publish` + URL swap; website mp4s are ffmpeg conversions; `public/assets/wiki/` is gitignored intermediates)
