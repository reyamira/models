...
- Grouped view state machine: `list_mode()` derives Grouped/Offerings/Flat from scope + `drill_key` + `flat_view`; `drill_name` is only the breadcrumb. Nav methods dispatch via `nav_len`/`nav_selected`/`nav_select`; canonical resolution uses `crate::labs` with the live `catalog.json` registry plus generated `data/models-dev-base-model-refs.json`. `set_lab_catalog` resolves a snapshot-wide identity map before search/filter/provider projection, so hiding a conflicting peer cannot change identity. Resolution is authoritative exact-ref → direct model id → provider/model id, then conservative anchor-backed canonical inference for leftovers, then a unique dual creator-qualified name + full-id lane for flattened namespaces. Remaining offerings peer-group sequentially on exact normalized name + leaf id, full id (flattened namespaces), then separator-compacted full id; each weaker lane sees only still-unlinked rows, and every whole multi-provider bucket must pass creator/output blockers. Exact authoritative pair, one-sided creator, and cross-record alias intersections are retained as shadow candidates only: they are audited with witness counts but never alter grouping. Per-offering provenance is retained; `≈` in grouped/provider/flat/drilled lists and the Identity detail row distinguish anchored canonical, creator-qualified canonical, peer, and models.dev-authored identity. Live `catalog.json` additions participate immediately, while new upstream TOML `base_model` edges require `mise run refresh-model-refs` and a binary release because the exact-ref artifact is pinned. `mise run audit-model-identity` runs provider-level held-out conformance plus live grouping/shadow distribution receipts
