...
- **Popups take precedence.** While a popup is open, the **wheel scrolls/navigates** it and a **left-click selects/toggles the row under the cursor** — the sort picker applies that sort (click-to-apply), the checkbox pickers (columns, agents/status trackers) toggle that row. Clicks outside the item rows are swallowed so nothing leaks to the panels behind; the help popup closes on click; the glossary has no rows (wheel-scroll only). Routed in `handle_modal_popup_mouse` / `handle_modal_popup_click` (`event.rs`); row mapping via `mouse::popup_row_at`. Popups render a fresh `ListState` each frame, so the scroll offset is deterministic (`offset = max(0, selected − (visible − 1))`) and `popup_row_at` reconstructs it without retained state — keep that invariant if you touch a picker's render.
