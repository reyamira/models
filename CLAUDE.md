...
- Benchmark pipeline (build-time, not in the app): the `transform` bin (`src/bin/transform/`, Cargo feature `pipeline`) converts raw upstream API/CSV dumps into the v2 schema (`transform aa|arena|epoch|llmstats`) → `data/v2/{aa,epoch,arena,llmstats}.json`, served via jsDelivr. The legacy v1 `data/benchmarks.json` lane (jq-only, AA) was **retired 2026-08-14**: nothing regenerates it, and the last file stays committed on main forever so pre-v2 released binaries degrade to **stale-not-broken** rather than 404. Never revive or reshape it. The original sunset criterion — wait for monthly jsDelivr hits on that file to decay from the ~1,035/month v2-launch baseline — was **abandoned as unanswerable**, not met: post-v2 the file sat at a flat ~47–48 hits/day (23 of 30 days inside a 46–52 band, ten at exactly 47) versus a noisy 13–79/day when real users still fetched it. A distribution that collapses onto two integers is scheduled machine traffic, and it sets a floor *above* whatever genuine pre-v2 launches remain, so the number could never fall. With the frozen file as a permanent fallback the traffic count doesn't bear on the decision anyway. See `data/stats/README.md`. `update-benchmarks.yml` is now multi-source: AA + LLM Stats run every trigger; Arena + Epoch are gated to twice daily (UTC hour 06/18 on the `:17` run), with a `workflow_dispatch` `refresh_all` boolean to bypass the gate. Each source step is `continue-on-error` so one transform failure leaves that source's previous file in place
...
- `src/config.rs` — user config file (agents, cache, display, aliases, benchmarks settings). `AliasesConfig` struct + `AliasKind` enum for symlink routing; `BenchmarksConfig.columns` persists the benchmarks tab's visible columns per source (metric ids plus namespaced metadata ids, not indices)
...
- `update-benchmarks.yml` — `workflow_dispatch`-only; multi-source. Checks out + builds the `transform` bin (Swatinem/rust-cache). Every trigger: AA (curl → `aa_raw.json` → `transform aa` → `data/v2/aa.json`; the step keeps its `id: aa` and its fetch because the v2 step is gated on `steps.aa.outcome` — the v1 jq lane it also used to write was retired 2026-08-14) and LLM Stats (bounded `/v1/rankings` + `/v1/models` fetch loop → `transform llmstats` → `data/v2/llmstats.json`). Twice-daily-gated (UTC hour 06/18 on the `:17` run, or `refresh_all` dispatch input): Arena (oolong-tea `latest.json` → 6 board JSONs → `transform arena`) and Epoch (epoch.ai ZIP → unzip → `transform epoch`). Each source step is `continue-on-error`; commit-if-changed covers `data/v2/` and **pushes through a rebase-retry loop** (`fetch-depth: 0` + `git pull --rebase --autostash`, 3 attempts) because `snapshot-stats.yml` also pushes to main and its daily cron can land in the same `:17` minute — a bare push loses that race with a non-fast-forward reject (observed 2026-08-06 11:17 UTC). Exhausting the retries fails the job deliberately: a swallowed push would report green while leaving the data unshipped and the CDN un-purged. jsDelivr purge per changed file (**retry/backoff** — jsDelivr's purge endpoint intermittently returns a 200 body of `no available server`, which curl can't auto-retry; without a successful purge an aliased `@main` URL serves stale "up to 7 days", so the app can show hours-old benchmark data even though git HEAD is fresh — the per-file purge loop retries 5× with backoff and is non-fatal, the next run re-purges). Triggered every 30 min (at `:17` and `:47` UTC) by the Cloudflare Worker in `infra/benchmark-trigger/` — the original GH `schedule:` cron was removed after proving unreliable under GitHub's cron throttling. `mise run refresh-sources` runs the same fetch+transform locally (`mise.toml` ↔ workflow stay in sync); the legacy AA-only `mise run refresh-benchmarks` task was removed with the v1 lane
- `snapshot-stats.yml` — daily `schedule:` + `workflow_dispatch`; runs `scripts/snapshot-stats.sh` (`mise run snapshot-stats`) to record jsDelivr per-file daily CDN hits into `data/stats/jsdelivr-history.json` as an app-launch proxy (the app has no telemetry; it fetches the v2 sources fresh per launch). jsDelivr serves only a rolling ~30-day window, so the script **upserts** the trailing 30 days — self-healing across dropped runs (tolerates GH cron throttling; no Cloudflare Worker needed). `fetch-depth: 0` + a 3-attempt rebase-before-push loop so it doesn't race the benchmark bot, failing the job when every attempt is rejected (the upsert self-heals a missed day, but only if persistent misses stay visible). Both push-to-main workflows now carry this same loop — a **third** pusher is the point to extract it into a shared script rather than hand-write a fourth copy. **Read the launch proxy as the median of `/data/v2/{arena,epoch,llmstats}.json` — never `aa.json`**, which external traffic has polled in isolated bursts since 2026-07 (3,614 hits on 2026-07-30 vs arena's 104, with the other three flat and no correlation to local activity); a median of three also survives a second file being adopted that way. The `data/stats/self-ping` sentinel and its `other-users ≈ …` subtraction were **retired 2026-08-14** (contaminated minuend; the sentinel only fired for wrapped interactive launches, never `cargo run`/`./target/release/models`) — existing self-ping rows are historical data, not a metric. Maintainer traffic is therefore **not separable** from these counts, and the archive measures **launches, not users**. See `data/stats/README.md`
...
- The v2 fetch+transform path is mirrored between `mise.toml` (`refresh-sources`) and the workflow's source steps — keep them in sync. (The old byte-identical-jq contract between `refresh-benchmarks` and the workflow died with the v1 lane on 2026-08-14.)
...
- The benchmarks data bot commits to main every ~30 min — branches that touch `data/v2/*.json` regrow merge conflicts on those files continuously; the resolution is always to take main's copy (both sides regenerate from the same upstream). `data/benchmarks.json` is now frozen and no longer conflicts
