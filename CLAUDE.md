...
### Tabs
- **Models Tab** (`src/tui/models/`) — browse models from models.dev API. **Grouped default view**: one row per model name (Model | Lab | Providers | price/context ranges), Enter drills into a model's per-provider offerings (breadcrumb + Esc back), `V` toggles the flat per-offering list (persisted), `p` opens a search-first provider modal (the provider sidebar is retired; layout is 60% list | 40% detail). Lab (canonical creator) is reconstructed from models.dev's `models.json` registry (`src/labs.rs` — no published endpoint exposes the `base_model` linkage). RTFO capability indicators (majority + dim-when-mixed on grouped rows); duplicate-row dimming in flat views is paused (grouping communicates sameness structurally)
