...
1. **Identity** — model name (White + BOLD, DarkGray if deprecated), model ID (DarkGray), provider + family + status row, then a **description** line (DarkGray, wrapped) from models.dev `description` (~100% coverage — omitted only when absent/empty)
