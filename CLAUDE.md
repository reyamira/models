...
### Tabs
- **Models Tab** (`src/tui/models/`) — browse models from models.dev API. **Grouped default view**: one row per canonical model when reconstructable (normalized-name fallback), with Model | Lab | Providers | price/context ranges; Enter drills into per-provider offerings (breadcrumb + Esc back), `V` toggles the flat per-offering list (persisted), `p` opens a search-first provider modal (the provider sidebar is retired; layout is 60% list | 40% detail). Lab/canonical identity comes from the canonical registry in `catalog.json` plus reconstruction in `src/labs.rs` — no served JSON exposes the provider TOMLs' exact `base_model` linkage. RTFO capability indicators (majority + dim-when-mixed on grouped rows); duplicate-row dimming in flat views is paused (grouping communicates sameness structurally)
...
### Data Flow
- Model data: the TUI fetches `catalog.json` at startup and on `r` refresh (`src/api.rs`) so provider offerings and the canonical model registry arrive as one coherent snapshot; provider-only CLI commands use the smaller `api.json`. `catalog.json` is exactly the combined `providers` + `models` payload and still omits provider TOML `base_model` links
