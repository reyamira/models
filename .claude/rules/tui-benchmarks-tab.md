...
- **State** (`BenchmarksApp`): `visible_columns: Vec<usize>` (metric indices, file order, default empty), plus picker state (`show_column_picker`, `column_picker_selected`, `column_picker_pending`).
- **Persistence**: saving the picker writes the selection to `config.toml` under `[benchmarks.columns]` as **metric ids per source id** (ids, not indices — Epoch's auto-prune shifts indices between pipeline runs), then `Config::save()` + status `Columns saved`. An empty selection removes the source's entry. On source switch / first load / refresh, `App::restore_saved_columns` re-resolves the saved ids onto indices in file order, silently dropping ids whose metric no longer exists.
...
**Identity block**: display name (White+BOLD), id (DarkGray), then 2-column `ColumnWidths` label-value rows (`[28%, 22%, 28%, 22%]`, 2-space indent), in order: Creator / Released, Reasoning / Effort, Weights / Context, **Region / Type, Tools / Output**, Variant (Variant only when present). Below `ColumnWidths::GRID_MIN_WIDTH` (46) the panel is too narrow for the 2×2 grid (`AnthropicReleased` collisions) and rows **stack** — one label/value pair per line (`cw.stacked`). In grid mode, a left value that overflows its column keeps a 1-space gap instead of running into the right label.
